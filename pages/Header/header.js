...
const getApeBalance = (rpcURL, nftAddress, nftAbi, address) => {
...
const getUserBalance = async (rpcURL, account) => {
    ...
    try {
        const balance = new BigNumber(await getEthBalance(account, web3)).div(10 ** 18)
        return balance.gt(0) ? balance : new BigNumber(0)
    } catch (e) {
        return new BigNumber("GET_BALANCE_ERR")
    }
}
...
const Header = () => {
    ...
    const connectWallet = async () => {
        ...
        try {
            ...
            const result = await getUserBalance(InfraURL, address);
            if (result.c !== undefined) {
                setUserBalance((result.c[0] / wei).toFixed(4))
            }
            const apeBalance = await getApeBalance(InfraURL, nftAdderss, abi, address);
            setUserApeBalance(userApeBalance);
            console.log(apeBalance, "apeBalance");
        } catch (error) {
            console.log(error)
        }
    }
    ...
    return (
        <>
            <div className="header">
                <p className="text-4xl cursor-pointer pt-4 ">ApeMart.io</p>
                <div className="inputarea">
                    <input className="inputText focus:outline-none appearance-none w-full text-sm leading-6 text-slate-900 placeholder-slate-400 rounded-md py-2 pl-10 ring-1 ring-slate-200 shadow-sm" type="text" aria-label="Filter projects" onKeyDown={handleKeyDown} value={inputFilter} onChange={(e) => { setInputFilter(e.target.value) }} placeholder="Example:Bored Ape Yacht Club, #3100, etc..." />
                </div>
                <div
                ...
                >
                    {account.address ? (
                        ...
                        >
                            <div>
                                <Text fontSize='xs' title={account.address}>{`${userApeBalance} APE`}</Text>
                                <Text fontSize='xs' title={account.address}>{`${userBalance} ETH`}</Text>
                            </div>
                            <div>
                                <img className="avatar rounded-full border border-gray-100 shadow-sm" name={account.ens} src="profile.png" mb='2' />
                            </div>
                        </Box>
                    ) : (
                        <Button type="button" onClick={connectWallet} className="wallet text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2">
                            <svg className="w-6 h-5 mr-2 -ml-1" viewBox="0 0 2405 2501" fill="none" xmlns="http://www.w3.org/2000/svg"> <g clipPath="url(#clip0_1512_1323)"> <path d="M2278.79 1730.86L2133.62 2221.69L1848.64 2143.76L2278.79 1730.86Z" fill="#E4761B" stroke="#E4761B" strokeWidth="5.94955" /> <path d="M1848.64 2143.76L2123.51 1767.15L2278.79 1730.86L1848.64 2143.76Z" fill="#E4761B" stroke="#E4761B" strokeWidth="5.94955" />
                                ...
                                <path d="M2285.34 919.338L1890.29 1081.17L1987.86 617.696L2285.34 919.338Z" fill="#763D16" stroke="#763D16" strokeWidth="5.94955" /> <path d="M1987.86 617.696L2400.16 570.1L2285.34 919.338L1987.86 617.696Z" fill="#763D16" stroke="#763D16" strokeWidth="5.94955" />
